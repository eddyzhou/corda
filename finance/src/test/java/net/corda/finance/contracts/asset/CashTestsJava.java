...
public class CashTestsJava {
    ...
    public void trivial() {
        transaction(tx -> {
            ...
            tx.tweak(tw -> {
                tw.unconstrainedOutput(CASH_PROGRAM_ID, () -> new Cash.State(issuedBy(DOLLARS(2000), defaultIssuer), new AnonymousParty(getMINI_CORP_PUBKEY())));
                tw.command(getMEGA_CORP_PUBKEY(), new Cash.Commands.Move());
                return tw.failsWith("the amounts balance");
            });

            tx.tweak(tw -> {
                tw.unconstrainedOutput(CASH_PROGRAM_ID,  () -> outState );
            ...
            tx.tweak(tw -> {
                ...
                tw.unconstrainedOutput(CASH_PROGRAM_ID, () -> outState.issuedBy(getMINI_CORP()));
                tw.command(getMEGA_CORP_PUBKEY(), new Cash.Commands.Move());
